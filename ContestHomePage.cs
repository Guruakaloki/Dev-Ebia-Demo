...
{
    public static class ContestHomePage
    {
        [FunctionName("ContestHomePage")]
        public static IActionResult Run(
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = "contesthomepage/{id}")]
                HttpRequestMessage req,
            [CosmosDB(
                databaseName: "contesthub",
                collectionName: "ContestHomePage",
                ConnectionStringSetting = "contesthub_DOCUMENTDB",
                PartitionKey ="/id",
                SqlQuery = "select c.contests from ContestHomePage c where upper(c.id)=upper({id})"
                )]
            ...
            TraceWriter log)
        {
            ...
            //}
            log.Info($"Processed request for {id} in ContestHomePage");
